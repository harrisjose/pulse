...
class G {
  constructor({
    ...
    t.forEach((s) => this.attachWindow(s)), require$$0.ipcMain.on(l, (s, a) => {
      Q({
        router: n,
        createContext: r,
        internalId: x(s, a),
...
const createRouter = (connectors, emitter2) => {
  ...
  return t.router({
    ...
    updateStatus: t.procedure.input(
      z.object({
        active: z.boolean()
      })
    ).mutation(async (opts) => {
      const { input } = opts;
      const promises = Object.values(connectors).map(
        (c2) => c2.setStatus(input.active)
      );
    ...
    subscribeStatus: t.procedure.subscription((opts) => {
      return observable((emit) => {
        ...
        emitter2.on("onStatusChange", onUpdate);
        return () => {
          emitter2.off("onStatusChange", onUpdate);
        };
      });
    })
  });
...
var newGithubIssueUrlExports = newGithubIssueUrl.exports;
var node = {};
const isElectron = "electron" in process.versions;
var isUsingAsar$1 = isElectron && require.main && require.main.filename && require.main.filename.includes("app.asar");
(function(exports) {
  exports.isElectron = "electron" in process.versions;
  exports.electronVersion = process.versions.electron || "0.0.0";
  exports.isUsingAsar = isUsingAsar$1;
  exports.fixPathForAsarUnpack = (path2) => exports.isUsingAsar ? path2.replace("app.asar", "app.asar.unpacked") : path2;
})(node);
const electron$1 = require$$0;
var api = new Proxy(electron$1, {
  get: (target, property) => target[property] || (target.remote ? target.remote[property] : void 0)
});
...
const isUsingAsar = isUsingAsar$1;
var is = {
...
var enforceMacosAppLocation;
var hasRequiredEnforceMacosAppLocation;
function requireEnforceMacosAppLocation() {
  if (hasRequiredEnforceMacosAppLocation)
    return enforceMacosAppLocation;
  hasRequiredEnforceMacosAppLocation = 1;
  const api$1 = api;
  const is$1 = is;
  enforceMacosAppLocation = () => {
    if (is$1.development || !is$1.macos) {
      return;
    }
    if (api$1.app.isInApplicationsFolder()) {
      return;
    }
    const appName = "name" in api$1.app ? api$1.app.name : api$1.app.getName();
    const clickedButtonIndex = api$1.dialog.showMessageBoxSync({
      type: "error",
    ...
    api$1.app.moveToApplicationsFolder({
      conflictHandler: (conflict) => {
        if (conflict === "existsAndRunning") {
          api$1.dialog.showMessageBoxSync({
            type: "error",
            message: `Another version of ${api$1.app.getName()} is currently running. Quit it, then launch this version of the app again.`,
            buttons: [
              "OK"
            ]
          });
...
var openSystemPreferences;
var hasRequiredOpenSystemPreferences;
function requireOpenSystemPreferences() {
  if (hasRequiredOpenSystemPreferences)
    return openSystemPreferences;
  hasRequiredOpenSystemPreferences = 1;
  const api$1 = api;
  const is$1 = is;
  openSystemPreferences = async (pane, section) => {
    if (is$1.macos) {
      await api$1.shell.openExternal(`x-apple.systempreferences:com.apple.preference.${pane}${section ? `?${section}` : ""}`);
    } else if (is$1.windows) {
      await api$1.shell.openExternal(`ms-settings:${pane}`);
    }
  };
  return openSystemPreferences;
}
(function(exports) {
  ...
  const path$1 = path;
  const electron2 = require$$0;
  const newGithubIssueUrl2 = newGithubIssueUrlExports;
  const node$1 = node;
  const api$1 = api;
  exports.api = api$1;
  const is$1 = is;
  exports.is = is$1;
  exports.electronVersion = node$1.electronVersion;
  ...
  const activeWindow = () => is$1.main ? electron2.BrowserWindow.getFocusedWindow() : electron2.remote.getCurrentWindow();
  exports.activeWindow = activeWindow;
  exports.runJS = (code, win = activeWindow()) => win.webContents.executeJavaScript(code);
  exports.fixPathForAsarUnpack = node$1.fixPathForAsarUnpack;
  exports.enforceMacOSAppLocation = requireEnforceMacosAppLocation();
  exports.menuBarHeight = () => is$1.macos ? api$1.screen.getPrimaryDisplay().workArea.y : 0;
  exports.getWindowBoundsCentered = (options) => {
    options = {
      window: activeWindow(),
      ...options
    };
    const currentDisplay = api$1.screen.getDisplayNearestPoint(api$1.screen.getCursorScreenPoint());
    const [width, height] = options.window.getSize();
    const windowSize = options.size || { width, height };
    const screenSize = options.useFullBounds ? currentDisplay.bounds : currentDisplay.workArea;
    const x2 = Math.floor(screenSize.x + (screenSize.width / 2 - windowSize.width / 2));
    const y2 = Math.floor((screenSize.height + screenSize.y) / 2 - windowSize.height / 2);
    return {
      x: x2,
      y: y2,
  ...
  if (is$1.main) {
    const isFirstAppLaunch = () => {
      const fs$1 = fs;
      const checkFile = path$1.join(api$1.app.getPath("userData"), ".electron-util--has-app-launched");
      if (fs$1.existsSync(checkFile)) {
        return false;
      }
      try {
        fs$1.writeFileSync(checkFile, "");
      } catch (error) {
        if (error.code === "ENOENT") {
          fs$1.mkdirSync(api$1.app.getPath("userData"));
  ...
  exports.darkMode = {
    get isEnabled() {
      ...
      return api$1.nativeTheme.shouldUseDarkColors;
    },
    onChange(callback) {
      if (!is$1.macos) {
        return () => {
  ...
  exports.openNewGitHubIssue = (options) => {
    const url2 = newGithubIssueUrl2(options);
    api$1.shell.openExternal(url2);
  };
  ...
  exports.showAboutWindow = (options = {}) => {
    const appName = api$1.app.getName();
    const appVersion = api$1.app.getVersion();
    ...
    api$1.app.setAboutPanelOptions(aboutPanelOptions);
    api$1.app.showAboutPanel();
  };
  exports.aboutMenuItem = (options = {}) => {
    ...
    return {
      label: `${options.title} ${api$1.app.getName()}`,
      click() {
        exports.showAboutWindow(options);
      }
    };
  ...
  exports.debugInfo = () => `
${api$1.app.getName()} ${api$1.app.getVersion()}
Electron ${exports.electronVersion}
${process.platform} ${os.release()}
Locale: ${api$1.app.getLocale()}
`.trim();
  exports.appMenu = (menuItems = []) => {
    return {
      label: api$1.app.getName(),
      submenu: [
        {
          role: "about"
        },
  ...
  exports.openSystemPreferences = requireOpenSystemPreferences();
})(electronUtil);
...
process.env.PUBLIC = require$$0.app.isPackaged ? process.env.DIST : path.join(process.env.DIST, "../public");
const emitter = new events.EventEmitter();
require$$0.app.on("ready", () => {
  const config = loadConfig();
  if (!Object.values(config.connectors).some((value) => {
    return value.enabled;
  })) {
    throw new Error("Atleast one connector needs to be enabled");
  }
  const enabled = Object.keys(config.connectors).filter(
    (k2) => config.connectors[k2].enabled
  );
  const connectors = {};
  enabled.forEach((c2) => {
    switch (c2) {
  ...
  require$$0.app.dock.hide();
  setInterval(() => {
    Object.values(connectors).forEach((c2) => c2.fetchStatus());
  }, 7e3);
});
